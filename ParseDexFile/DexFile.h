...
//uleb128 is an encode style, just use const u1* to represent it
typedef const u1* uleb128;
...
// 	encoded_method virtual_methods[virtual_methods_size];
	//just Symbolize, 1 represent Corresponding length, so can't use this struct directly
	encoded_field static_fields[1];
	encoded_field instance_fields[1];
	encoded_method direct_methods[1];
	encoded_method virtual_methods[1];
};
...
struct DexFile {
	...
	/*set values for all member fields*/
	void initall();
	//unsigned int parse
	void getContMapLists();
	//Get All String Id and Print 
	void getContString_ids();
	//Get All Type Id and Print
	void getContType_ids();
	//Get All Proto Id and Print
	void getContProto_ids();
	//Get All Field Info and Print
	void getContField_ids();
	//Get All Method Info and Print
	void getContMethod_ids();
	//Get All Class_defs Info and Print
	void getContClass_defs();
	/*
		Get Some Designated Item and Print
	*/
	const u1* getDexString(int nIndex);
	const u1* getDexType(int nIndex);
	void printDexProto(int nIndex, char pszBuf[1024]);
	void printDexProto(int nIndex);
	void printField_ids(int nIndex);
	void printMethod_ids(int nIndex);
	//用于参数多个时，Proto的参数类型（type列表排列）解析
	void parseTypelist(DexTypeList* pTypelist);
	//以下函数为解析出Class_data_item中的内容，最终打印出方法对应的字节码
	void parseClass_data_item(const u1* pDataItemStrat);
	void parseEncoded_field(u4 filed_idx, u4 access_flags);
	void parseEncoded_method(u4 method_idx, u4 access_flags, u4 code_off);
	void parseDexCode(DexCode* pDexCode);
	void parseAnnotations_directory_item(DexTypeList* pTypelist);
	void parseDexEncodedArray(DexTypeList* pTypelist);
};
